...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        const ReachedAddress = await queryInterface.sequelize.define(
            'reachedaddress',
            {
                address: {
                    type: Sequelize.STRING(44),
        ...
        const updateMaps = (
            ...
            if (previousValues === undefined) {
                ...
                newValues = {
                    ...
                    volume: new BigNumber(previousValues.volume)
                        .plus(value)
                        .toString(),
                    reach:
                        reachedAddresses.get(
                            ...
                            ? previousValues.reach
                            : previousValues.reach + 1,
                };
            }
            // add to BeneficiaryTransaction
            getBlockTime(blockHash).then((timestamp) =>
        ...
        for (let b = 0; b < beneficiaryAddresses.length; b++) {
            ...
            const rawResult = query.data.result.filter((r) =>
                new BigNumber(r.value.toString()).gt('9999999999999999')
            ); // >0,009
        ...
        accVolumeTxReachByDay.set(
            uniqueOrderedDates[0],
            volumeTxReachByDay.get(uniqueOrderedDates[0])
        );
        for (let x = 1; x < uniqueOrderedDates.length; x += 1) {
            const today = volumeTxReachByDay.get(uniqueOrderedDates[x]);
            const previous = accVolumeTxReachByDay.get(
                uniqueOrderedDates[x - 1]
            );
            accVolumeTxReachByDay.set(uniqueOrderedDates[x], {
                transactions: today.transactions + previous.transactions,
                volume: new BigNumber(today.volume)
                    .plus(previous.volume)
                    .toString(),
