...
export async function verifyUserSuspectActivity(): Promise<void> {
    ...
    update_trust
        AS (UPDATE "user" SET suspect = false WHERE suspect = true AND address NOT IN (SELECT * FROM suspect))
    UPDATE "user" SET suspect = true WHERE address IN (SELECT * FROM suspect)`;
