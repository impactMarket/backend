    ...
    async up(queryInterface, Sequelize) {
        ...
        for (let index = 0; index < records.length; index++) {
            await CommunityState.update(
                { raised: records[index].total },
                { where: { communityId: records[index].communityId } }
            );
        }
        ...
        records = (
            await queryInterface.sequelize.query(
                `select "communityId", count(amount) c, sum(amount) total
        from claim
        group by "communityId"`,
                { raw: true }
            )
        ...
        for (let index = 0; index < records.length; index++) {
            await CommunityState.update(
                { claimed: records[index].total, claims: records[index].c },
                { where: { communityId: records[index].communityId } }
            );
        }
        ...
        for (let index = 0; index < records.length; index++) {
            lastTx = await CommunityState.update(
                { beneficiaries: records[index].total },
                { where: { communityId: records[index].communityId } }
            );
        }
        return lastTx;
