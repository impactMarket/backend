...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        const SSI = await queryInterface.sequelize.define(
            'ssi',
            {
                id: {
                    type: Sequelize.INTEGER,
        ...
        for (let c = 0; c < allCommunities.length; c++) {
            const allSSI = await SSI.findAll({
                where: { communityPublicId: allCommunities[c].publicId },
            });

            for (let index = 0; index < allSSI.length; index++) {
                const ssi = allSSI[index];
                const recentSSIs = allSSI.slice(
                    Math.max(index - 5, 0),
                    index + 1
                );
                const sumSSI = recentSSIs.reduce(
                    (acc, mssi) => acc + mssi.ssi,
                    0
                );
                ...
                if (recentSSIs.length !== 0) {
                    ssiToInsert.push({
                        ...
                        ssi:
                            Math.round(
                                parseFloat(
                                    (sumSSI / recentSSIs.length).toFixed(2)
                                ) * 100
                            ) / 100,
                        ubiRate: 0,
                        estimatedDuration: 0,
