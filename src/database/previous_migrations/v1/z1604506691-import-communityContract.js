...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        for (let index = 0; index < allCommunityAdded.length; index++) {
            ...
            const community = await Community.findOne({ where: { contractAddress: communityAdded.values._communityAddress } });
            if (community !== null) {
                communityContractsToInsert.push({
                    ...
                    incrementInterval: parseInt(communityAdded.values._incrementInterval),
                    createdAt: new Date(),
                    updatedAt: new Date()
                });
            }
        }
        return queryInterface.bulkInsert('communitycontract', communityContractsToInsert);
    },
