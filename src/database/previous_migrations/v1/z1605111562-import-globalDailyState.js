    ...
    up: async (queryInterface, Sequelize) => {
        ...
        const allCommunityDailyState = await CommunityDailyState.findAll({
            attributes: [
                ...
                [
                    Sequelize.fn('sum', Sequelize.col('beneficiaries')),
                    'totalBeneficiaries',
                ],
        ...
        const countDays = await CommunityDailyMetrics.findAll({
            attributes: [[Sequelize.fn('count', Sequelize.col('ssi')), 't']],
            where: {
                communityId: { [Sequelize.Op.in]: onlyPublicValidCommunities },
            },
            group: ['date'],
        ...
        for (let index = 0; index < 2; index++) {
            ...
            totalRaised = new BigNumber(totalRaised)
                .plus(state.totalRaised)
                .toString();
            totalDistributed = new BigNumber(totalDistributed)
                .plus(state.totalClaimed)
                .toString();
            // totalBackers = totalBackers + parseInt(state.totalBackers); // TODO: fix calculation
            totalBeneficiaries =
                totalBeneficiaries + parseInt(state.totalBeneficiaries);
            totalVolume = '0';
            totalTransactions = 0;
        ...
        for (let index = 0; index < total; index++) {
            const allSSIs = (
                await CommunityDailyMetrics.findAll({
                    ...
                    where: {
                        communityId: {
                            [Sequelize.Op.in]: onlyPublicValidCommunities,
                        },
                    ...
                    where: {
                        txAt: {
                            ...
                            [Sequelize.Op.gte]: new Date(
                                new Date(state.date).getTime() - 2592000000
                            ), // 30 * 24 * 60 * 60 * 1000
                        },
                    },
                    raw: true,
