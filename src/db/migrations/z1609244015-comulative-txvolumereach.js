...
module.exports = {
    async up(queryInterface, Sequelize) {
        ...
        records = (await queryInterface.sequelize.query('select date, sum(amount) volume, count(tx) txs, count(distinct "withAddress") reach from beneficiarytransaction where date < \'' + new Date().toISOString().split('T')[0] + '\' group by date order by date asc', { raw: true }))[0];

        const comulative = [records[0]];
        for (let index = 1; index < records.length; index++) {
            const previousElement = comulative[index - 1];
            const element = records[index];
            const uniqueReach = (await queryInterface.sequelize.query('select count(distinct "withAddress") reach from beneficiarytransaction where date <= \'' + new Date(element.date).toISOString().split('T')[0] + '\'', { raw: true }))[0];
            comulative.push({
                date: element.date,
                volume: new BigNumber(element.volume).plus(previousElement.volume).toString(),
                txs: new BigNumber(element.txs).plus(previousElement.txs).toString(),
                reach: uniqueReach[0].reach,
            });
        }
        
        for (let x = 0; x < comulative.length; x += 1) {
            await GlobalDailyState.update({
                totalVolume: comulative[x].volume,
                totalTransactions: BigInt(comulative[x].txs),
                totalReach: BigInt(comulative[x].reach),
