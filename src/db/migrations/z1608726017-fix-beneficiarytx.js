...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        const addBeneficiaryTx = async (fromBeneficiary, beneficiary, address, value, timestamp, tx) => {
            try {
        ...
        for (let b = 0; b < beneficiaryAddresses.length; b++) {
            ...
            const rawResult = query.data.result.filter((r) => r.value.toString().length > 15); // >0,0009 (999999999999999)

            for (let r = 0; r < rawResult.length; r++) {
                // 0xdC553892cdeeeD9f575aa0FBA099e5847fd88D20 atestation proxy
                if (rawResult[r].to.toLowerCase() === '0xdC553892cdeeeD9f575aa0FBA099e5847fd88D20'.toLowerCase()) {
                    continue;
                }
                // not to himself
                ...
                if (
                    !communitiesAddresses.includes(ethers.utils.getAddress(rawResult[r].from)) &&
                    !communitiesAddresses.includes(ethers.utils.getAddress(rawResult[r].to))
                ) {
                    // if from beneficiary
