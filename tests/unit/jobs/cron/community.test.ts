...
import { validNonEmptyMonthLongCommunities, communitiesContract, ssiLast4Days, totalClaimedLast30Days, communityIds, validNonEmptyLessThanMonthLongCommunities, validEmptyCommunities } from '../../../fake/community';
...
describe('[jobs - cron] community', () => {
    ...
    describe('#calcuateCommunitiesMetrics()', () => {
        ...
        it('all valid, non-empty communities, 30+ days', async () => {
            listFullStub.returns(Promise.resolve(validNonEmptyMonthLongCommunities));
            ...
            assert.calledWith(dailyMetricsAdd.getCall(0), validNonEmptyMonthLongCommunities[0].publicId, 1.75, 1.25, 1.18, 42.37, match.any);
            assert.calledWith(dailyMetricsAdd.getCall(1), validNonEmptyMonthLongCommunities[1].publicId, 2.45, 2.11, 0.3, 66.66, match.any);
            assert.calledWith(dailyMetricsAdd.getCall(2), validNonEmptyMonthLongCommunities[2].publicId, 1.06, 1.05, 0.44, 22.72, match.any);
        });

        it('all valid, non-empty communities, <30 days', async () => {
            listFullStub.returns(Promise.resolve(validNonEmptyLessThanMonthLongCommunities));
