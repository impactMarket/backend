...
export const verifyProposals = async (event: any, context: any) => {
  try {
    ...
    if(newProposals.length > 0) {
      const data = await client.getAllByType(process.env.PRISMIC_CUSTOM_TYPE!, { lang: process.env.PRISMIC_LANGUAGE });  
      ...
      const promises = newProposals.map((proposal) =>
        axios.post(process.env.DISCORD_WEBHOOK!, {
          embeds: [{
            type: 'rich',
            title: title,
            description: description.replace('{id}', parseInt(proposal.id)),
