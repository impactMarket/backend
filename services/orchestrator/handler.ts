...
const getHerokuConfigVars = async (appId: string) => {
    const response = await axios.get(`https://api.heroku.com/apps/${appId}/config-vars`, {
        headers: {
...
export const herokuTrigger = async (event: any, context: any) => {
    try {
        ...
        const herokuConfigVars = await getHerokuConfigVars(requestBody.data.app.id);
        ...
        listFunctions.Functions?.forEach(async (res) => {
            const config = await lambda.getFunctionConfiguration({ FunctionName: res.FunctionName! }).promise();
